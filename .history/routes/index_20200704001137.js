...
passport.use(new LocalStrategy({
    ...
    function (res, email, password, done, next, render) {
        console.log(email, password, done);
        const sqlDatabase = require('../db.js');


        sqlDatabase.query('SELECT * FROM users WHERE email = ?', [email], async (err, results, fields, res, next, render) => {
            if (err)
                return done(err);
            ...
            if (results.length === 0) {
                return res.render('login', { message: 'This is a test notification.' });
