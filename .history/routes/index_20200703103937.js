...
passport.use(new LocalStrategy({
    ...
    function (req, email, password, done, next) {
        ...
        sqlDatabase.query('SELECT * FROM users WHERE email = ?', [email], async (err, results, req, res, fields) => {
            if (err) {
            ...
            if (results.length === 0) {
                ...
                await bcrypt.compare(password, hash, function (err, response, res, req) {
