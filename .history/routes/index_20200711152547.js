...
router.get("/superhero-movies", authenticationMiddleware(), function(req, res) {
    ...
    // sqlDatabase.query("SELECT users.username, comments.comments, comments.date FROM users , comments WHERE users.id=comments.id AND users.id = ",
    sqlDatabase.query("SELECT users.username, comments.comments, comments.date FROM users INNER JOIN comments ON users.id=comments.id WHERE users.id AND comments.id = id",
        ...
        function(error, results, fields) {
            ...
            results.forEach((id) => {
                console.log('My data: ' + id.username, id.date, id.comments);
                // console.log(results)
                // console.log(fields);
