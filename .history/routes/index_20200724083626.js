...
tion(req, email, password, done) {
    ...
    atabase.query('SELECT * FROM users WHERE email = ?', [email], (err, results, fields, res) => {
            err) return done(err); console.log(err, fields, results);
...
router.post("/register", function(req, res, next) {
    ...
    atabase.query('SELECT * FROM users WHERE email =?', [email], async(error, results, ) => {
        error) {
        console.log(error);
    } else
        results.length > 0) {
