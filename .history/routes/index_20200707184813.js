...
router.post("/superhero-movies", function (req, res) {
    ...
    sqlDatabase.query('SELECT * FROM users WHERE id = ?', [comments], function (err, results, fields, res) {
        if (err) throw err;
        console.log(results);
    });
...
sqlDatabase.query('INSERT INTO users (username, email, password, comments) VALUES (?, ?, ?)',
    [username, email, comments], function (error, results, fields) {
