...
router.post("/superhero-movies", function (req, res) {
    ...
    // sqlDatabase.query('INSERT INTO users  WHERE(username, email, password, comments) VALUES (?, ?, ?)',
    // [username, email, hash], function (error, results, fields) {
        
        sqlDatabase.query('SELECT SELECT CURRENT_USER as id',  async function (error, results, fields, err) {
    ...
    sqlDatabase.query('INSERT id INTO users (comments) VALUES (?)',
    [comments], function (error, results, fields) {
            if (error) throw error;
            console.log(results);
