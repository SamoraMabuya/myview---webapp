...
router.get("/superhero-movies", authenticationMiddleware(), function(req, res) {
    ...
    sqlDatabase.query("SELECT users.username, comments.comments, comments.date FROM users INNER JOIN comments ON users.user_id=comments.user_id WHERE id = id", [user, comments],
        function(error, results, fields) {
            if (error) throw error;
