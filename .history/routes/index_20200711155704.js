...
router.get("/superhero-movies", authenticationMiddleware(), function(req, res) {
    ...
    sqlDatabase.query("SELECT users.username, comments.comments, comments.date FROM users INNER JOIN comments ON users.id=comments.id WHERE email = 'SESSION_ID'", [email],
        ...
        sqlDatabase.query('SELECT * FROM users WHERE email = ?', [email], (err, results, fields, res) => {
            ...
            if (results.length === 0) {
                ...
                bcrypt.compare(password, hash, function(err, response) {
                    console.log(user_id.email + 'we aere her');
                    if (response === true) {
                        return done(null, user_id.email + 'we aere her');
                    } else {
                        console.log(user_id.email + 'we aere her');
                        console.log(err);
