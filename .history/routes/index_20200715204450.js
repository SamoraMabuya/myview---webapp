...
router.get("/superhero-movies", authenticationMiddleware(), function(req, res) {
    ...
    sqlDatabase.query("SELECT LAST_INSERT_ID() users.username, comments.comments, comments.date FROM users INNER JOIN comments ON users.user_id=comments.user_id", [user],
        function(error, results, fields) {
            if (error) throw error;
