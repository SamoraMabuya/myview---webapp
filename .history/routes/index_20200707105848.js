...
router.get("/superhero-movies", function (req, res) {
    sqlDatabase.query('SELECT * FROM users', (err, results, fields) => {
        if (err) throw err;
        console.log(err);
        res.render('superhero-movies', {userData:results});
        Handlebars.registerHelper("userResults", function(items) {
            var html = "<ul>";
            userData.forEach(function(results) {
              html += "<li>" + results.username + "</li>";
              html += "<li>" + results.date + "</li>";
